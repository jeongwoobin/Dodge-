...
struct _rank rank[11] = { NULL,  };		//랭킹구조체 선언, 초기화
...
void addRank() {
	...
	while(1) {
		system("cls");
		printFrame();
		gotoxy(20, 10);  printf("GAME SET");
		gotoxy(20, 11);  printf("랭킹을 저장하시겠습니까? (Y/N)>>"); 	scanf_s("%c", &save, 1); scanf("%c", &num2);
		if (save == 'Y' || save=='y') {
				//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
				gotoxy(13, 11);  printf("랭킹을 저장할 이니셜을 입력하세요");
				gotoxy(13, 12); printf("영어 대문자, 3글자까지 가능) >> ");
				scanf_s("%s", rank[10].name, strlen(rank[10].name));			//scnaf서식에 띄어쓰기잇으면 공백이 아닌문자가 올때까지 다 버려라
				//system("pause");
				break;
			}
		gotoxy(20, 12);  printf("잘못입력하셨습니다.");
	}
	rankSort();			//랭킹정렬
	inputRank();
}
