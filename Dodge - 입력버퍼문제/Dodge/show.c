...
boolean gameOver(struct _bullet* bullet) {
	if (checkCrush(bullet) == 1) { // user와 bullet들 중 하나라도 부딪히면 break;
		...
		//gotoxy(13, 12);  printf(">> ");
		//scanf_s("%s", rank[10].name, 11);
		///*if (sizeof(rank[10].name) > 11) {
		//	printf("10글자 이하로 입력해야합니다.");
		//}*/
		//rankSort();			//랭킹정렬
		//inputRank();
		temp = 0;
	...
	if (disappearBullets(bullet) == 1) {  // bullet들이 모두 소멸되면 break;
		...
		gotoxy(13, 11);  printf("랭킹을 저장할 닉네임을 입력하세요(10글자까지 공백없이 입력가능)");
		gotoxy(13, 12);  printf(">> ");
		scanf(" %s", rank[10].name);			//scnaf서식에 띄어쓰기잇으면 공백이 아닌문자가 올때까지 다 버려라
		/*if (sizeof(rank[10].name) > 11) {
			printf("10글자 이하로 입력해야합니다.");
		}*/
		rankSort();			//랭킹정렬
...
void rankSort() {
	struct _rank temp[1] = { 0, 0 };
	for (int i = 0; i < sizeof(rank) - 1; i++) {				//버블소트
		for (int j = 0; j < sizeof(rank) - 2; j++) {
